...
type Props={shipment?:Shipment;clients:DirectoryOption[];carriers:DirectoryOption[];isDemo:boolean};

export function ShipmentForm({shipment,clients,carriers,isDemo}:Props){
  ...
  return <form onSubmit={submit} className="grid gap-6 xl:grid-cols-[1fr_320px]">
    <Card><CardContent className="grid gap-5 sm:grid-cols-2">
      ...
      {currency!=="PLN"?<Field label={`1 ${currency} in reporting currency (PLN)`} name="exchange_rate_to_base"><Input id="exchange_rate_to_base" name="exchange_rate_to_base" type="number" min="0.0001" step="0.0001" required defaultValue={shipment?.exchangeRateToBase??4.28} disabled={isDemo}/></Field>:<input type="hidden" name="exchange_rate_to_base" value="1"/>}
